...
async function getRecipesId() {
  ...
  const dataFetch = await fetch(
    `https://api.spoonacular.com/recipes/complexSearch?query=${searchValue}${apiKey}`,
    {
      method: "GET",
      headers: {
  ...
  buttonsToSave.forEach((btn, index) => {
    ...
    btn.addEventListener("click", (e) => {
      ...
      if (buttonTarget.classList.contains(`recipe${indexOfRecipe}`)) {
        ...
        addToStorage(
          infoArray[index].image,
          infoArray[index].title,
          infoArray[index].sourceUrl
        );
      }
    });
    searchInput.value = "";
...
function addRecipe(recipeObject) {
  ...
  const savedPin = document.createElement("p");
  savedPin.classList.add("saved-pin");
  savedPin.innerHTML = `<i class='fas fa-thumbtack'></i>`;
  savedPin.classList.add("saved-pin");
  library.appendChild(savedPin);
  ...
  savedLink.innerHTML = `<i class='fas fa-link'></i>`;
